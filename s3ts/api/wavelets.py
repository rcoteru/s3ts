...
class CWT(nn.Module):
    ...
    def forward(self, x):
        ...
        if self._kernel.is_complex():
            ...
            # )
            output_real = nn.functional.conv2d(x, self._kernel_real, stride=self.stride, padding="same")
            output_imag = nn.functional.conv2d(x, self._kernel_imag, stride=self.stride, padding="same")
            ...
            # )
            output = nn.functional.conv2d(x, self._kernel, stride=self.stride, padding="same")
